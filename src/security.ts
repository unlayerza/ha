...
export function canonicalMessage(id:string,kind:string,from:string,term:bigint,sentAt:number,payload:unknown){
  return JSON.stringify([id,kind,from,term.toString(),sentAt,payload],(_,value)=>typeof value==="bigint"?value.toString():value)
}
