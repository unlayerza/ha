...
export function canonicalMessage(id:string,kind:string,from:string,term:bigint,sentAt:number,payload:unknown,configurationVersion?:bigint){
  return JSON.stringify([id,kind,from,term.toString(),configurationVersion?.toString()??null,sentAt,payload],(_,value)=>typeof value==="bigint"?value.toString():value)
}
