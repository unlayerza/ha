...
export class Election{
  ...
  private async startPreVote(){if(!this.electionAllowed||!this.isVoter(this.nodeId))return;this.role="candidate";this.preVotes=new Set([this.nodeId]);this.resetDeadline();const req={from:this.nodeId,term:this.term+1n,lastTerm:this.term,lastIndex:0,configurationVersion:this.hooks.configurationVersion()};await this.send("pre_vote_request",req)}
  private async startElection(){if(!this.electionAllowed||!this.isVoter(this.nodeId))return;this.role="candidate";this.term++;this.votedFor=this.nodeId;this.votes=new Set([this.nodeId]);this.preVotes.clear();await this.hooks.persist(this.term,this.votedFor);await this.hooks.onTerm(this.term);await this.hooks.onRole(this.role,this.term);this.resetDeadline();const req:VoteRequest={from:this.nodeId,term:this.term,lastTerm:this.term,lastIndex:0};await this.send("vote_request",{...req,configurationVersion:this.hooks.configurationVersion()})}
  ...
  private async onPreVoteResponse(m:ClusterMessage){const r=m.payload as PreVoteResponse;if(this.role!=="candidate"||!r.granted||r.term!==this.term||r.configurationVersion!==this.hooks.configurationVersion()||!this.isVoter(r.from))return;this.preVotes.add(r.from);if(this.preVotes.size>=this.majority())await this.startElection()}
