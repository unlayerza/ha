...
export class Election{
  ...
  private deadline=0;private timer?:Timer;private votes=new Set<string>();private preVotes=new Set<string>();private lastHeartbeat=0;private random:()=>number;
  ...
  private majority(){return Math.floor(this.hooks.members().length/2)+1}
  ...
  private async tick(){if(this.role==="leader"){if(this.clock.now()-this.lastHeartbeat>=this.heartbeatMs){this.lastHeartbeat=this.clock.now();await this.broadcastHeartbeat()}return}if(this.clock.now()>=this.deadline)await this.startPreVote()}
  private async startPreVote(){
    this.role="candidate";this.preVotes=new Set([this.nodeId]);this.resetDeadline();
    const req={from:this.nodeId,term:this.term+1n,lastTerm:this.term,lastIndex:0};
    await this.send("pre_vote_request",req)
  }
  private async startElection(){
    this.role="candidate";this.term++;this.votedFor=this.nodeId;this.votes=new Set([this.nodeId]);this.preVotes.clear();
    await this.hooks.persist(this.term,this.votedFor);await this.hooks.onTerm(this.term);await this.hooks.onRole(this.role,this.term);this.resetDeadline();
    const req:VoteRequest={from:this.nodeId,term:this.term,lastTerm:this.term,lastIndex:0};await this.send("vote_request",req)
  }
  ...
  private async onPreVoteRequest(m:ClusterMessage){
    const r=m.payload as any;const candidateTerm=BigInt(r.term);const healthyLeader=this.leaderId!==null&&this.clock.now()-this.lastHeartbeat<this.minElectionMs;
    const grant=candidateTerm>=this.term+1n&&!healthyLeader;
    const resp:PreVoteResponse={from:this.nodeId,term:this.term,granted:grant};
    const out={id:id("msg"),kind:"pre_vote_response",from:this.nodeId,fromAddress:m.fromAddress||m.from,term:this.term,sentAt:this.clock.now(),payload:resp,signature:""} as ClusterMessage;
    try{await this.transport.send(m.fromAddress||m.from,out)}catch{}
  }
  private async onPreVoteResponse(m:ClusterMessage){
    const r=m.payload as PreVoteResponse;if(this.role!=="candidate"||!r.granted||r.term!==this.term)return;
    this.preVotes.add(r.from);if(this.preVotes.size>=this.majority())await this.startElection()
  }
  ...
  private async onVoteRequest(m:ClusterMessage){
    const r=m.payload as VoteRequest;let grant=false;if(r.term===this.term&&(this.votedFor===null||this.votedFor===r.from)){grant=true;this.votedFor=r.from;await this.hooks.persist(this.term,this.votedFor);this.resetDeadline()}
    const resp:VoteResponse={from:this.nodeId,term:this.term,granted:grant,reason:grant?undefined:"already_voted"};const out={id:id("msg"),kind:"vote_response",from:this.nodeId,fromAddress:m.fromAddress||m.from,term:this.term,sentAt:this.clock.now(),payload:resp,signature:""} as ClusterMessage;try{await this.transport.send(m.fromAddress||m.from,out)}catch{}
  }
