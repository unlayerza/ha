...
export class Election{
  ...
  private async onHeartbeat(m:ClusterMessage){if(!this.isVoter(m.from))return;const claimedLeader=m.from;if(this.role==="leader"&&claimedLeader!==this.nodeId){this.role="follower";this.leaderId=claimedLeader;this.votes.clear();this.preVotes.clear();this.lastHeartbeat=this.clock.now();this.resetDeadline();await this.hooks.onRole(this.role,this.term);return}this.leaderId=claimedLeader;this.lastHeartbeat=this.clock.now();this.resetDeadline();if(this.role!=="follower"){this.role="follower";this.preVotes.clear();this.votes.clear();await this.hooks.onRole(this.role,this.term)}}
