...
**Join configuration ordering hardening:** the leader now delivers the initial configuration proposal directly to the joining process before broadcasting it, and the initial join path no longer waits on a separate configuration snapshot before starting the v0→v1 proposal. This removes an unnecessary startup dependency and makes the bootstrap join handshake deterministic; the process quorum test remains the validation gate.
